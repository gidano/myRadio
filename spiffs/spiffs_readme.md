...
A `SPIFFS_xxx.bin` fájl feltölthető például:
...
Természetesen továbbra is lehetőség van a `./data/` mappa tartalmát kézzel feltölteni, de az előre elkészített `SPIFFS_xxx.bin` használata gyorsabb és egyszerűbb megoldás.

### Javaslat

Ha most telepíted a firmware-t, érdemes a megfelelő `SPIFFS_xxx.bin` fájlt is rögtön feltölteni, így a rendszer azonnal használható lesz.
